...
mod round_trip {
    ...
    fn round_trip() {
        for testcase in TEST_VECTORS.iter()
            .filter(|x| x.raw_hashtype == 0 || x.raw_hashtype == 8) {
            let hashtype = match testcase.raw_hashtype {
                0 => HashType::P2PKH,
                8 => HashType::P2SH,
                _ => panic!("found bad raw hashtype")
            };
            let payload: Payload = testcase.cashaddr.parse().unwrap();
            let recon = payload.encode(testcase.prefix, hashtype).expect("Encoding Failed");
    ...
    fn round_trip_backward() {
        for testcase in TEST_VECTORS.iter() {
            let payload = Payload {
                ...
                hash_type: match testcase.raw_hashtype {
                    0 => HashType::P2PKH,
                    8 => HashType::P2SH,
                    _ => continue
                },
